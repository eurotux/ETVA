...
#use Utils;
use Data::Dumper;
#TODO verificar restart aos servicos
...
sub server_restore{ 
    `tar -xzvf backup.tgz` or retErr("ETMS Restore", "Cannot untar file: $!", '1');
    ...
    unlink "backup_ldap.ldif";
    `make -C /srv/qmail/control/` or retErr("ETMS Restore", "Cannot run make on qmail configuration files: $!", '1');
...
sub server_info{
        #find server status. Up if timer >15 sec.
...
sub fill{
    ...
    my $alias = MyDispatcher::select_alias('', %params); 
