...
try:
    from RhevCredServer import RhevCredServer
except ImportError:
    # The RhevCredServer doesn't exist in RHEL-5. So we provide a
...
class CommandHandlerLinux:
    ...
    def lock_screen(self):
        cmd = [ '/usr/share/rhev-agent/rhev-locksession' ]
    ...
    def shutdown(self, timeout, msg):
        ...
        cmd = [ '/usr/share/rhev-agent/rhev-shutdown', '-h', "+%d" % (delay), "\"%s\"" % (msg) ]
        logging.debug("Executing shutdown command: %s", cmd)
        subprocess.call(cmd)
    
    def etasp_read(self, method):
        cmd = '/usr/bin/perl /srv/guestagent/ga_etasp.pl -H localhost -m ' + method
        logging.debug("Executing remote call to ETASP API: %s", cmd)
