...
elsif (\`cat /etc/redhat-release 2>/dev/null\` =~ /Scientific\\s+Linux.*\\s+release\\s+(\\S+)/i && \$1 >= 4 && \$1 < 6) {
	print "oscheck='Scientific Linux'\\n";
	}
elsif (\`cat /etc/redhat-release 2>/dev/null\` =~ /Scientific\\s+Linux.*\\s+release\\s+(\\S+)/i && \$1 >= 6) {
	print "oscheck='Scientific Linux'\\n";
	}
...
elsif (\`cat /etc/redhat-release 2>/dev/null\` =~ /Red.*Hat\\s+Enterprise\\s+Linux\\s+(Server|Client)\\s+release\\s+(\\d+)/i) {
	print "oscheck='Redhat Enterprise Linux'\\n";
	}
...
elsif (\`cat /etc/system-release 2>/dev/null\` =~ /Amazon\\s+Linux.*\\s2011/) {
