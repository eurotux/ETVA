...
get/set virtual agent uniq id
    ...
    my $uuid = VirtAgent->getuid();
    VirtAgent->setuid( $uuid );
...
sub setconfig {
    ...
    $CONF = ref($H) ? $H : \%conf;
    $UUID = $CONF->{'uuid'};
    Utils::set_conf($CONF->{'CFG_FILE'},%$CONF);
}
sub loadconf {
    $CONF = Utils::get_conf();
    $UUID = $CONF->{'uuid'};
...
sub getDomainInfo {
    ...
    my $dom;
    eval {
        $dom = $vm->get_domain_by_name($name);
    };
    if( $@ ){
        return retErr('_GET_DOMAIN_',"Can't get domain info: $@");
    }
...
sub vmMigrate {
    ...
    if( $uri || $p{'daddr'} ){
        eval {
            $dcon =  Sys::Virt->new( uri=>$uri, address=>$p{'daddr'} );
        };
        if( $@ ){
    ...
    my $ddom;
    eval {
        $ddom = $dom->migrate($dcon,$flags,$dname,$uri,$bw);
    };
    if( $@ ){
