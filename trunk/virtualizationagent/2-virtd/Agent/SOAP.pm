...
Agent::SOAP - Agent class for treat SOAP calls

=head1 SYNOPSIS

    my $Agent = Agent::SOAP->new( Port=>$port, ..., debug=>1, cm_uri=>..., _register_handler=>$reghandler, _alarmhandler_=>$keepalivehandler );
...
=item new
    ...
    my $Agent = Agent::SOAP->new( Port=>$port, ..., debug=>1, cm_uri=>..., _register_handler=> sub { ... } );
...
sub processdata {
    ...
    plog("Agent::SOAP processing data");

    # Get data
    my $soap_request = $self->receive($fh);

    plog("processdata: $soap_request") if($debug);
...
sub treatRequest {
    ...
    if( $@ ){
plog "Failed while unmarshaling the request: $@" if( $debug );
...
sub response_soap_fault {
    ...
    plog("SOAP_FAULT: faultcode: $faultcode, faultstring: $faultstring, detail: $result_desc") if( $debug );

    my %a = ( faultcode => encode_content($faultcode),
                faultstring => encode_content($faultstring),
                detail => encode_content($result_desc) );
    ...
    plog("response_soap_fault: $response") if($debug);
...
sub response_soap {
    ...
    my %a = ( result=>encode_content($res,1) );
    ...
    plog("response_soap: $soap_response") if($debug);
