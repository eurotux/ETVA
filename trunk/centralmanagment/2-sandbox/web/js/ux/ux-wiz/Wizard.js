...
// -------- helper
    ...
    initPanels : function()
    {
        ...
        this.westPanel = new Ext.ux.Wiz.West(this.westConfig);
     ...
     *
     */
    addCard:function(pos,card){

        var cards = this.cards;
                       
        if(this.getCardPosition(card)!=-1) return;

        if(pos >= cards.length) pos = cards.length;
        ...
        (this.westPanel).addStep(last_index,this.cards[last_index]);

        for(len = cards.length; i < len;i++)
                (this.westPanel).updateStep(i);
                        
        this.cardPanel.insert(pos,card);
        (this.headPanel).addStep(last_index,card);
                               
        card.wizard = this;

        card.on('show', this.onCardShow, this);
        card.on('hide', this.onCardHide, this);
        card.on('clientvalidation', this.onClientValidation, this);
        
    },
    removeCard:function(card)
    {
        var pos = this.getCardPosition(card);
        
        this.cardPanel.remove(card);        
        this.cards.splice(pos,1);

        (this.headPanel).removeStep(pos,card);
        (this.westPanel).removeStep(pos,card);

        var i = pos;
        for(len = this.cards.length; i < len;i++)
            (this.westPanel).updateStep(i);
    },
...
// -------- listeners
    ...
    onPreviousClick : function()
    {
        ...
        this.cards[this.currentCard].fireEvent('previousclick', this.cards[this.currentCard]);        
    ...
    onNextClick : function()
    {
        ...
        this.cards[this.currentCard].fireEvent('nextclick', this.cards[this.currentCard]);
