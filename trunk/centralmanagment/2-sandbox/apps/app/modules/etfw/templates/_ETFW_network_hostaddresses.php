<script>
    ...
    Ext.ns('ETFW_Network.HostAddresses');

    ETFW_Network.HostAddresses.Grid = Ext.extend(Ext.grid.GridPanel, {
        initComponent:function() {
            ...
            var dataStore = new Ext.data.JsonStore({
                url: this.url,
                baseParams:{id:this.serviceId,method:'list_hosts'},
                id: 'id',
                remoteSort: false,
                totalProperty: 'total',
                root: 'data',
            ...
            // call parent
            ETFW_Network.HostAddresses.Grid.superclass.initComponent.apply(this, arguments);
            ...
            this.store.on('update',function(store,rec,op){
                        ...
                        conn.request({
                            url: this.url,
                            params:{id:this.serviceId,method:method,params:Ext.encode(send_data)},
        ...
        deleteData : function (items) {
            ...
            for(var i=0,len = items.length;i<len;i++){
                ...
                conn.request({
                    url: this.url,
                    params:{id:this.serviceId,method:'delete_host',params:Ext.encode(data)},
                    failure: function(resp,opt){
                        var response = Ext.util.JSON.decode(resp.responseText);
                        Ext.MessageBox.alert('Error Message', response['info']);
                        Ext.ux.Logger.error(response['error']);
    ...
    Ext.reg('etfw_network_hostaddresses_grid', ETFW_Network.HostAddresses.Grid);



    ETFW_Network.HostAddresses.Main = function(serviceId,containerId) {



        ETFW_Network.HostAddresses.Main.superclass.constructor.call(this, {
    ...
    Ext.extend(ETFW_Network.HostAddresses.Main, Ext.Panel, {
    });
