...
User.List.Grid = Ext.extend(Ext.grid.GridPanel, {
    id: 'user-list-grid',
    border:false,
    autoScroll:true,
    stripeRows: true,    
    ...
    initComponent:function(){
        ...
        this.cm = new Ext.grid.ColumnModel([                    
                    {header: __("Id"), dataIndex: 'id',width:40},
                    {header: __("User Name"), id:'username', width: 200, sortable: true, dataIndex: 'username',renderer: function (value, metadata, record, rowIndex, colIndex, store) {
                                metadata.attr = String.format('ext:qtip="{0}"',<?php echo json_encode(__('Right-click to open sub-menu or double-click to edit')) ?>);
                                return value;
                            }},
                    {header: __("Created At"), dataIndex: 'createdAt', width: 140, sortable: true,renderer: this.renderDate},
                    {header: __("Last Login"), dataIndex: 'lastLogin', width: 140, sortable: true,renderer: this.renderDate},
                    {header: __("Active"), dataIndex: 'isActive', width: 60, sortable: true},
                    {header: __("Super Admin"), dataIndex: 'isSuperAdmin', width: 80, sortable: true}
        ]);
        ...
        this.store = new Ext.data.JsonStore({
            autoLoad: true,
            proxy: new Ext.data.HttpProxy({url: <?php echo json_encode(url_for('user/jsonGrid')); ?>}),
            id: 'Id',
            totalProperty: 'total',
            root: 'data',
        ...
        this.tbar = [{
                        ...
                        ,url:<?php echo json_encode(url_for('user/View_UserCreationWizard')); ?>
                        ,call:'User.Create'
        ...
        User.List.Grid.superclass.initComponent.call(this);
