...
class SOAP_Transport_TCP extends SOAP_Transport
{
     ...
     */
    var $socket = null;
    var $rcv_timeout = false; //receive response timeout
    ...
    function _socket_ping()
    {
        ...
        //if (!$this->socket) {
            ...
            /* tweak. make timeout on receiving response */
            if($this->rcv_timeout){
                //socket_set_nonblock($this->socket);
                socket_set_option($this->socket, SOL_SOCKET, SO_RCVTIMEO, array("sec" => $this->rcv_timeout, "usec" => 0));
            }
            // Connect.
    ...
    function send($msg, $options = array())
    {
        ...
        if(isset($options['rcv_timeout'])) $this->rcv_timeout = $options['rcv_timeout'];
