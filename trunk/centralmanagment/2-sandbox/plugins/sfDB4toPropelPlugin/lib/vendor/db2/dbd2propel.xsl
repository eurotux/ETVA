...
==== Version: 0.4-sg (2006-11-13) (http://blog.tooleshed.com/?p=6)
==== Description:
...
<xsl:template match="/">
    ...
    <xsl:attribute name="package">lib.model</xsl:attribute>
		<xsl:apply-templates />
	</database>
</xsl:template>
...
<xsl:template name="get_datatype">
    ...
    <xsl:when test="$type = 'MEDIUMTEXT'" >LONGVARCHAR</xsl:when> 
    <xsl:when test="$type = 'LONGTEXT'" >LONGVARCHAR</xsl:when> 
