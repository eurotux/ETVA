...
class serverResumeTask extends sfBaseTask
{
  protected function configure()
  {
    ...
    $this->name             = 'resume';
    $this->briefDescription = 'resume the server';
    $this->detailedDescription = <<<EOF
The [server:resume|INFO] task does things.
Call it with:

  [php symfony server:resume|INFO]
EOF;
  }
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    $this->log("[INFO] Resume server with '$server'");
    ...
    if(!$etva_server){
        ...
        if(!$etva_node){
            ...
            $msg_i18n = Etva::makeNotifyLogMessage(sfConfig::get('config_acronym'),
                                                                    ...
                                                                    EtvaServerPeer::_ERR_RESUME_, array('name'=>$server));
            $error = array('success'=>false,'agent'=>sfConfig::get('config_acronym'),'error'=>$msg_i18n,'info'=>$msg_i18n);
        ...
        $response = $server_va->send_resume($etva_node);
