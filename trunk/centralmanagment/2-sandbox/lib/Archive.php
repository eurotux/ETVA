...
/*
 * Modified version:
 *  - added stream capability (stream option)
 */
...
class Archive
{
	...
	function create_archive()
	{
		...
		if ($this->options['inmemory'] == 0)
		{
            ...
            if($this->options['stream']){
                $socket_h = $this->options['socket'];
                $this->archive = $socket_h;
            }else if ($this->options['overwrite'] == 0 && file_exists($this->options['name'] . ($this->options['type'] == "gzip" || $this->options['type'] == "bzip" ? ".tmp" : "")))
			{
		...
		case "gzip":
            ...
            if($this->options['stream'] == 0 && (!$this->options['count'] || $this->options['count'] == 0))
            {
                if (!$this->create_gzip())
                {
		...
		if ($this->options['inmemory'] == 0 && $this->options['stream'] == 0)
		{
			fclose($this->archive);            
	...
	function add_data($data)
	{
        ...
        if($this->options['count']){
            ...
            if($this->options['type'] == "gzip") $this->count+=strlen(gzencode($data, $this->options['level']));
            else $this->count+=strlen($data);
//        if ($this->options['stream']){
//            if($this->options['type'] == "gzip") echo gzencode($data, $this->options['level']);
//            else echo $data;
//
//            ob_flush();
//            flush();
//        }
                ...
                $bytes_sent = fwrite($this->archive, gzencode($data, $this->options['level']));
            }
            else if ($this->options['inmemory'] == 0)
                fwrite($this->archive, $data);                                            
            else
                $this->archive .= $data;

        if($this->options['cbk']){
            $funct = $this->options['cbk'];
            call_user_func_array($funct, array($bytes_sent));
        }
	}
